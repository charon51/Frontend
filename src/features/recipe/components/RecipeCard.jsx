...
const RecipeCard = ({ recipe }) => {
    ...
    return (
        <div
            className={`p-3 flex gap-3 border hover:cursor-pointer hover:bg-slate-100 ${(recipe?.id === currentId) ? "bg-orange-100" : "bg-white"}`}
            onClick={handleClickCard}
        >
            <img src={recipe?.image} alt="food" width={140} height={140} />
            <div className="flex flex-col flex-1 justify-between">
                <h6 className="text-orange-500 text-lg font-bold">{recipe?.label}</h6>
                ...
                <div className="flex justify-between items-center">
                    ...
                    <p>{recipe?.ingredientLines?.length} ingredientes</p>
                </div>
                <div className="flex items-center gap-1">
                    {"Fuente: "}
                    <Link to={recipe?.url} target="_blank" rel="noopener noreferrer" className="flex items-center font-bold text-blue-500 hover:text-blue-700">
